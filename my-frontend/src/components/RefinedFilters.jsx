...
const RefinedFilters = () => {
        ...
        <div className="absolute z-20 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-2xl max-h-60 overflow-auto animate-in slide-in-from-top-2 fade-in duration-200">
          {options.map((option, index) => (
            <button
              ...
              className={`w-full px-4 py-3 text-sm text-left hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 transition-all duration-200 transform hover:scale-[1.02] animate-in slide-in-from-left-1 fade-in ${selected === option ? 'bg-gradient-to-r from-indigo-50 to-purple-50 text-indigo-700 font-medium border-l-4 border-indigo-500' : 'text-gray-700'
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div className={`bg-white/90 backdrop-blur-sm border border-gray-200 rounded-xl shadow-sm p-6 transition-all duration-700 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2 px-3 py-1 bg-gradient-to-r from-indigo-100 to-purple-100 rounded-full animate-in zoom-in duration-300">
            ...
            <span className="text-sm font-medium text-indigo-700">Live Updates</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            {showCustomRange && (
              <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200 animate-in slide-in-from-top-2 fade-in duration-300">
        ...
        <div className="mt-6 pt-4 border-t border-gray-200">
          <div className="flex items-center justify-between text-sm text-gray-600">
            ...
            <span className="text-indigo-600 font-medium">{filteredDataPoints} transactions loaded</span>
          </div>
        </div>
      </div>
    </div>
